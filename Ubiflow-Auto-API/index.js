...
// TEMPORAIRE — essai ciblé sur les 4 lots gpt-5-nano encore en échec (mots interdits résiduels
// après 2 tentatives). Trois pistes combinées, uniquement pour ce modèle de test (aucun effet
// sur callOpenAILmnp/gpt-4o en production) : 3e tentative, message de correction avec
// alternatives toutes prêtes plutôt qu'une simple mention du mot fautif, et reasoning_effort
// relevé sur les tentatives de correction (gpt-5-nano n'accepte pas de température réglable —
// seule la valeur par défaut est permise — donc reasoning_effort est le levier le plus proche de
// "plus littéral/moins rapide" disponible pour ce modèle). À retirer une fois l'essai terminé.
...
// mécanique plutôt qu'une reformulation libre, moins de nuance à interpréter pour un petit modèle.
// Reprend l'exemple déjà donné dans le prompt V2 lui-même pour la famille "garanti*"/loyer.
...
async function callOpenAILmnpGpt5Nano(textContext, base64Images, lot) {
    const donneesFiables = donneesFinancieresFiablesDepuisLot(lot);
    const residenceType = lot.program?.residenceType || null;
    const libelleCategorie = residenceType ? LIBELLES_CATEGORIE_RESIDENCE[residenceType] || null : null;
    ...
    let resultat, hits = [], usageTotal = { prompt_tokens: 0, completion_tokens: 0 }, essaisUtilises = 0;
    const MAX_TENTATIVES_CONFORMITE = 3;
    for (let essai = 1; essai <= MAX_TENTATIVES_CONFORMITE; essai++) {
        essaisUtilises = essai;
        // Effort de raisonnement minimal à la génération initiale (rapide, pas cher) — relevé à
        // "low" sur les tentatives de correction : gpt-5-nano n'accepte pas de température
        // réglable (valeur par défaut uniquement), donc pas de vrai équivalent "plus littéral"
        // par ce biais ; reasoning_effort est le levier le plus proche disponible pour ce modèle.
    ...
    return { titre: resultat.titre, texte: resultat.texte, usage: usageTotal, alerteConformite: hits.length > 0 ? hits : null, essaisUtilises };
}
