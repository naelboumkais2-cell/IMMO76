...
- Référence de publication : LMNP auto-générée par promoteur reconnu (`{Initiales}-{VILLE}-{n°lot}`, 4 partenaires connus dans `promoteurs.js`), `INT-{VILLE}-{n°lot}` si le promoteur est "La Centrale du LMNP" (comparaison exacte sur `program.developer.name`), Neuf via une référence de programme saisie manuellement une fois par l'agence (Réglages → "Références de programme (Neuf)", table `programmes_reference`, clé `program.id` Otaree). Les trois sont bien transmises jusqu'à Hubiflow (`reference_generee` → `hubiflowClientReel.js` → `buildUbiflowPayload`).
- **Exclusion automatique des promoteurs LMNP tiers** (`promoteurLmnpExclu`, `referenceGenerator.js`) : un lot LMNP dont le promoteur n'est ni un des 4 partenaires connus ni "La Centrale du LMNP" est exclu de l'auto-publication (filtré dans `autoGenererEtPublier`, jamais proposé sur l'écran de confirmation) — demande explicite du client, "on ne le diffusera pas". Le lot reste importé normalement, visible/traitable manuellement si besoin (Supervision) — jamais invisible ni supprimé.
