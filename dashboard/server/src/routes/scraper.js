...
// TEMPORAIRE — audit qualité LMNP (test symétrique 2026-09-24). Lecture seule. À retirer après.
scraperRouter.get('/diag-audit-textes', exigerConnexion, async (req, res) => {
    try {
        ...
        const rows = await db.prepare(
            `SELECT a.id, a.ville, a.type_bien, a.prix, a.surface, a.donnees_ia, a.raw_data,
                    a.images, a.reference_generee, p.nom AS portail, ap.maj_le
             FROM annonces a
        ...
        res.json(rows.map((r) => {
            ...
            return {
                ...
                reference: r.reference_generee, typeBien: r.type_bien,
                natureLabel: raw?.natureLabel, residenceType: raw?.program?.residenceType ?? null,
                programNom: raw?.program?.name ?? null,
                surface: r.surface, prixBase: r.prix,
                loyerMensuel: raw?.prices?.[0]?.monthlyRent ?? null,
                titreGenere: ia.titre, texte: ia.texte,
